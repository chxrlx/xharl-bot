...
*${usedPrefix + command}*  credit [cantidad] [@user]
...
`.trim()
  ...
  if (user[type] * 1 < count) return m.reply(`✳️  *${type}*  insuficiente`)
  let confirm = `
...
`.trim()
  ...
  confirmation[m.sender] = {
    ...
    count,
    timeout: setTimeout(
      () => (m.reply('⏳ El tiempo se acaba'), delete confirmation[m.sender]),
      60 * 1000
    ),
  }
}
...
handler.before = async m => {
  ...
  if (/yes?/g.test(m.text.toLowerCase())) {
    ...
    if (previous > user[type] * 1 && _previous < _user[type] * 1)
      m.reply(
        `Transacción exitosa ✅ \n\n*₹${count}* fue transferido a @${(to || '').replace(/@s\.whatsapp\.net/g, '')}`,
        null,
        { mentions: [to] }
      )
    else {
      ...
      m.reply(
        `❎ La transferencia falló *${count}* ${type} a *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`,
        null,
        { mentions: [to] }
      )
    }
